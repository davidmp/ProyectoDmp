...
function updatePersonList(transaction,results) {
    ...
    if (results.rows.length > 0) {
      for (i = 0; i < results.rows.length; i++) {
        ...
        //var dd="create_person.html?idPersona="+row.idpersona+"";
        ...
        template +=  "<td> <input type=button onclick='urlRedirectX(" + row.idpersona + ");' class= 'button-primary' value=E> <input type=button onclick='deletepersona(" + row.idpersona + ");' class= 'button-success' value=X> </td></tr>";
      }
      if (tabla ) {
...
function urlRedirectX(id){
    console.log("ssssurl");
    //alert(url);
    window.location="create_person.html?idPersona="+id;
}
